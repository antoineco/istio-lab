...
# Smoke test the hello-world application using a custom host name.
function helloapp::smoke_test_hostname {
	local cluster_name=$1
	local namespace=$2
	local hostname=$3
	...
	# Because we are (most likely) dealing with "fake" host names which
	# aren't resolvable by the application, we need to communicate with the
	# proxy directly by setting 'http_proxy'. This assumes that a suitable
	# listener and ip:port binding was configured in the proxy, e.g. via a
	# Sidecar object.
	#
	# Sleep before sending HTTP requests to give kubectl enough time to
	# attach and avoid missing some output.
	while IFS= read -r line; do
		...
		--image=curlimages/curl@sha256:9fab1b73f45e06df9506d947616062d7e8319009257d3a05d970b0de80a41ec5 `# tag:7.85.0` \
		--rm \
		--env='http_proxy=http://localhost' `# uses default socks port 1080` \
		poke-custom-host -- \
		sh -c '
			sleep 3;
